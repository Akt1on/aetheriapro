  ...
  if (!project) {
    return (
      <div className="dark relative flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-6 text-center text-foreground">
        ...
        <p className="text-white/70">Возможно, проект ещё не опубликован.</p>
  ...
  return (
    <div className="dark relative min-h-screen bg-background text-foreground">
      ...
      <header className="mx-auto max-w-5xl px-6 pt-10">
        <Link to="/" className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/70 transition-colors hover:text-white">
      ...
      <section className="mx-auto max-w-5xl px-6 pb-14 pt-14 sm:pt-24">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
          <div className="text-[10px] uppercase tracking-[0.35em] text-white/70">{project.category} · {project.year}</div>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <div className="text-[10px] uppercase tracking-[0.35em] text-white/70">Другие кейсы</div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {others.map((p) => (
            ...
            >
              <div>
                <div className="text-[10px] uppercase tracking-[0.3em] text-white/70">{p.category}</div>
                <div className="mt-1 font-display text-xl text-white">{p.name}</div>
              </div>
              <ArrowUpRight className="h-5 w-5 shrink-0 text-white/70 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-white" />
            </Link>
          ))}
        </div>
        ...
        <div className="mt-14 flex flex-col items-start gap-5 rounded-3xl border border-white/10 p-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-white/70">Соберите проект в конфигураторе — оценка за минуту.</p>
          </div>
          <Link
            to="/"
            hash="configurator"
